...
def _exp_disk_weight(R, z, Rd, z0):
    """
    Unnormalized 3D disk density shape: exp(-R/Rd) * exp(-|z|/z0).
    Constants cancel after normalization, so only shape matters.
    """
    return np.exp(-R / Rd) * np.exp(-np.abs(z) / z0)

def generate_disk_equal_mass(
    N, M_tot, Rd, z0, Rmax=None, zmax=None, seed=None):
    """
...
def generate_disk_importance_mass(
    N, M_tot, Rd, z0, Rmax=None, zmax=None, max_mass_ratio=None, seed=None):
    """
...
pos2, vel2, mass2, meta2 = generate_disk_importance_mass(N, M_tot, Rd, z0, max_mass_ratio=10, seed=42)
